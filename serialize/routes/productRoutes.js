...
router.get('/delivery', (req, res, next) => {
    ...
    }).populate('area_and_delivery', 'area No_of_delivery').exec().then(doc => {
        res.status(200).json({
            _id: doc._id,
...
router.get('/jobs', (req, res, next) => {
    Jobs.findOne({
        _id: req.query.jobid
    }).populate('order').exec().then(doc => {
        Order.findOne({
            _id: doc.order
        }).populate('order_items').exec().then(doco => {
            res.status(200).json({
                pickup_point: {
...
router.get('/delivery_jobs', (req, res, next) => {
    Delivery.findOne({
        ...
        }).populate('pending_jobs').populate('completed_jobs').exec().then(docs => {
            console.log(docs);
                ...
                Person.findOne({
                    ...
                    .then(sales => {
                        ...
                        nodeoutlook.sendEmail({
                            ...
                            html: message,
                            attachments: [{
                                'filename': 'SalesOrder_Medicento_' + docp.pharma_name + '_' + sales.user.useremail + '_' + date.toISOString() + '.csv',
                                'content': csv
                            }]
                        });
